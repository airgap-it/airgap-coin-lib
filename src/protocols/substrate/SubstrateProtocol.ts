...
export abstract class SubstrateProtocol extends NonExtendedProtocol implements ICoinDelegateProtocol {
  ...
  public async getPublicKeyFromMnemonic(mnemonic: string, derivationPath: string, password?: string): Promise<string> {
    const keyPair = await this.options.config.accountController.createKeyPairFromMnemonic(mnemonic, derivationPath, password)
  ...
  public async getPublicKeyFromHexSecret(secret: string, derivationPath: string): Promise<string> {
    const keyPair = await this.options.config.accountController.createKeyPairFromHexSecret(secret, derivationPath)
  ...
  public async getAddressFromPublicKey(publicKey: string): Promise<string> {
    return this.options.config.accountController.createAddressFromPublicKey(publicKey)
  }
  ...
  public async getTransactionsFromAddresses(addresses: string[], limit: number, offset: number): Promise<IAirGapTransaction[]> {
    ...
    const txs = await Promise.all(
      addresses.map((address) => this.options.config.blockExplorerClient.getTransactions(address, limit, pageNumber))
    )
  ...
  public async signWithPrivateKey(privateKey: Buffer, rawTransaction: RawSubstrateTransaction): Promise<string> {
    const txs = this.options.config.transactionController.decodeDetails(rawTransaction.encoded)
    const signed = await Promise.all(
      txs.map((tx) => this.options.config.transactionController.signTransaction(privateKey, tx.transaction, tx.payload))
    )

    txs.forEach((tx, index) => (tx.transaction = signed[index]))

    return this.options.config.transactionController.encodeDetails(txs)
  }
  ...
  public async getBalanceOfAddresses(addresses: string[]): Promise<string> {
    const balances = await Promise.all(addresses.map((address) => this.options.config.accountController.getBalance(address)))
  ...
  public async getAvailableBalanceOfAddresses(addresses: string[]): Promise<string> {
    const balances = await Promise.all(
      addresses.map((address) => this.options.config.accountController.getTransferableBalance(address, false))
    )
    const balance = balances.reduce((current: BigNumber, next: BigNumber) => current.plus(next))
  ...
  public async estimateMaxTransactionValueFromPublicKey(publicKey: string, recipients: string[], fee?: string): Promise<string> {
    ...
    const feeEstimate = await this.options.config.transactionController.estimateTransactionFees(futureTransactions)
  ...
  public async estimateFeeDefaultsFromPublicKey(
    ...
    const transaction = await this.options.config.transactionController.createTransaction(SubstrateTransactionType.TRANSFER, publicKey, 0, {
      to: destination && destination.length > 0 ? destination : publicKey,
      value: new BigNumber(value)
    })
    const fee = await this.options.config.transactionController.calculateTransactionFee(transaction)
  ...
  public async prepareTransactionFromPublicKey(
    ...
    const encoded = await this.options.config.transactionController.prepareSubmittableTransactions(
      publicKey,
      available,
  ...
  public async broadcastTransaction(encoded: string): Promise<string> {
    const txs = this.options.config.transactionController.decodeDetails(encoded).map((tx) => tx.transaction)

    try {
      const txHashes = await Promise.all(
        txs.map((tx, index) =>
          this.options.config.nodeClient.submitTransaction(tx.encode()).catch((error) => {
            error.index = index
            throw error
          })
        )
  ...
  public async getCurrentDelegateesForPublicKey(publicKey: string): Promise<string[]> {
    return this.options.config.accountController.getCurrentValidators(publicKey)
  }

  public async getCurrentDelegateesForAddress(address: string): Promise<string[]> {
    return this.options.config.accountController.getCurrentValidators(address)
  }

  public async getDelegateeDetails(address: string): Promise<DelegateeDetails> {
    const validatorDetails = await this.options.config.accountController.getValidatorDetails(address)
  ...
  public async isPublicKeyDelegating(publicKey: string): Promise<boolean> {
    return this.options.config.accountController.isNominating(publicKey)
  }

  public async isAddressDelegating(address: string): Promise<boolean> {
    return this.options.config.accountController.isNominating(address)
  }
  ...
  public async getDelegatorDetailsFromAddress(address: string): Promise<DelegatorDetails> {
    return this.options.config.accountController.getNominatorDetails(address)
  }
  ...
  public async getDelegationDetailsFromAddress(address: string, delegatees: string[]): Promise<DelegationDetails> {
    const [nominatorDetails, validatorsDetails] = await Promise.all([
      this.options.config.accountController.getNominatorDetails(address, delegatees),
      Promise.all(delegatees.map((validator) => this.options.config.accountController.getValidatorDetails(validator)))
    ])
  ...
  public async estimateMaxDelegationValueFromAddress(address: string): Promise<string> {
    const results = await Promise.all([
      this.options.config.accountController.getTransferableBalance(address),
  ...
  private async getTransactionDetailsFromEncoded(encoded: string): Promise<IAirGapTransaction[]> {
    const txs = this.options.config.transactionController.decodeDetails(encoded)
  ...
  private async getFutureRequiredTransactions(
    ...
    const results = await Promise.all([
      this.options.config.accountController.isBonded(publicKey),
      this.options.config.accountController.isNominating(publicKey),
      this.options.config.accountController.getTransferableBalance(publicKey)
    ])
