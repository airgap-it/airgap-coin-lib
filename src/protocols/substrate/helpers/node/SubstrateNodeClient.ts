...
export class SubstrateNodeClient {
    ...
    public async getAccountInfo(address: SubstrateAddress): Promise<SubstrateAccountInfo | null> {
        return this.fromStorage('System', 'Account', SCALEAccountId.from(address, this.network))
            .then(item => item ? SubstrateAccountInfo.decode(this.network, item) : null)
    }
    ...
    public async getBonded(address: SubstrateAddress): Promise<SubstrateAddress | null> {
        return this.fromStorage('Staking', 'Bonded', SCALEAccountId.from(address, this.network))
            .then(item => item ? SCALEAccountId.decode(this.network, item).decoded.address : null)
    }

    public async getNominations(address: SubstrateAddress): Promise<SubstrateNominations | null> {
        return this.fromStorage('Staking', 'Nominators', SCALEAccountId.from(address, this.network))
            .then(item => item ? SubstrateNominations.decode(this.network, item) : null)   
    }
    ...
    public async getRewardPoints(eraIndex: number): Promise<SubstrateEraRewardPoints | null> {
        return this.fromStorage('Staking', 'ErasRewardPoints', SCALEInt.from(eraIndex, 32))
            .then(item => item ? SubstrateEraRewardPoints.decode(this.network, item) : null)
    }
    ...
    public async getStakersClipped(eraIndex: number, validator: SubstrateAddress): Promise<SubstrateExposure | null> {
        return this.fromStorage('Staking', 'ErasStakersClipped', SCALEInt.from(eraIndex, 32), SCALEAccountId.from(validator, this.network))
    ...
    public async getRewardDestination(address: SubstrateAddress): Promise<SubstratePayee | null> {
        return this.fromStorage('Staking', 'Payee', SCALEAccountId.from(address, this.network))
            .then(item => item 
    ...
    public async getStakingLedger(address: SubstrateAddress): Promise<SubstrateStakingLedger | null> {
        return this.fromStorage('Staking', 'Ledger', SCALEAccountId.from(address, this.network))
            .then(item => item ? SubstrateStakingLedger.decode(this.network, item) : null)
    }

    public async getValidators(): Promise<SubstrateAddress[] | null> {
        return this.fromStorage('Session', 'Validators')
            .then(items => items 
                ? SCALEArray.decode(this.network, items, SCALEAccountId.decode).decoded.elements.map(encoded => encoded.address)
                : null
            )
    }
    ...
    public async getIdentityOf(address: SubstrateAddress): Promise<SubstrateRegistration | null> {
        return this.fromStorage('Identity', 'IdentityOf', SCALEAccountId.from(address, this.network))
            .then(item => item ? SubstrateRegistration.decode(this.network, item) : null)
    }

    public async getValidatorPrefs(eraIndex: number, address: SubstrateAddress): Promise<SubstrateValidatorPrefs | null> {
        return this.fromStorage('Staking', 'ErasValidatorPrefs', SCALEInt.from(eraIndex, 32), SCALEAccountId.from(address, this.network))
    ...
    public async getActiveEraInfo(): Promise<SubstrateActiveEraInfo | null> {
        return this.fromStorage('Staking', 'ActiveEra')
            .then(item => item ? SubstrateActiveEraInfo.decode(this.network, item) : null)
    }
