...
export class SubstrateBlockExplorerClient {
    ...
    public async getTransactions(address: string, size: number, pageNumber: number): Promise<Partial<IAirGapTransaction[]>> {
        ...
        return response.data.data
            ...
            .map(transfer => {
                const destination = SubstrateAddress.fromPublicKey(transfer.attributes.destination.id).toString()
                return {
                    from: [SubstrateAddress.fromPublicKey(transfer.attributes.sender.id).toString()],
                    to: [destination],
