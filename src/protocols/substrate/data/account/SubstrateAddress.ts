...
// If changed, the test address in `test/protocols/specs/substrate.ts` must be changed accordingly
enum SS58_Format {
    POLKADOT_LIVE = 0,
    KUSAMA = 2,
    SUBSTRATE = 42
...
export class SubstrateAddress {
    ...
    public static from(accountId: SubstrateAccountId): SubstrateAddress {
    ...
    public static fromPublicKey(payload: Buffer | Uint8Array | string, format: SS58_Format = SS58_Format.KUSAMA): SubstrateAddress {
    ...
    public compare(other: SubstrateAccountId): number {
        return this.payload.compare(SubstrateAddress.from(other).payload)
    }
