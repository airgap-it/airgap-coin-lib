...
export class SubstrateTransaction extends SCALEClass {
    public static create(type: SubstrateTransactionType, config: SubstrateTransactionConfig): SubstrateTransaction {
        return new SubstrateTransaction(
            ...
            SubstrateTransactionMethod.create(type, config.methodId.moduleIndex, config.methodId.callIndex, config.args)
        )
    }
    ...
    public static fromTransaction(transaction: SubstrateTransaction, config?: Partial<SubstrateTransactionConfig>): SubstrateTransaction {
        return new SubstrateTransaction(
            ...
            (config && config.args && config.methodId) 
                ? SubstrateTransactionMethod.create(transaction.type, config.methodId.moduleIndex, config.methodId.callIndex, config.args) 
                : transaction.method
        )
    }

    public static decode(type: SubstrateTransactionType, raw: string): SCALEDecodeResult<SubstrateTransaction> {
