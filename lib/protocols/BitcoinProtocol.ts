...
export class BitcoinProtocol implements ICoinProtocol {
  ...
  constructor(network: INetwork = bitcoinJS.networks.bitcoin, baseApiUrl = 'https://insight.bitpay.com', bitcoinJSLib = bitcoinJS) {
    ...
    this.socket = socketIo(baseApiUrl)
  }
  ...
  getTransactionDetails(transaction): IAirGapTransaction {
  ...
  startLedgerWatcher() {
    this.socket.on('connect', () => {
      console.log('connection succeeded, subscribing for blocks')
      // Join the room.
      this.socket.emit('subscribe', 'inv')
    })
    this.socket.on('block', async blockHash => {
      const initialResponse = await axios.get(`${this.baseApiUrl}/api/txs/?pageNum=2block=${blockHash}`, { responseType: 'json' })
      for (let i = 0; i < initialResponse.data.pagesTotal; i++) {
        await axios.get(`${this.baseApiUrl}/api/txs/?pageNum=${i}&block=${blockHash}`, { responseType: 'json' }).then(response => {
          const transactions = response.data
          const airGapTransactions: IAirGapTransaction[] = []

          for (let transaction of transactions.txs) {
            let tempAirGapTransactionFrom: string[] = []
            let tempAirGapTransactionTo: string[] = []
