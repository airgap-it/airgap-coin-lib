...
export class AirGapMarketWallet extends AirGapWallet {
  ...
  public fetchCurrentMarketPrice(baseSymbol = 'USD'): Promise<BigNumber> {
    return new Promise((resolve, reject) => {
      cryptocompare.price(this.coinProtocol.symbol.toUpperCase(), baseSymbol)
        .then(prices => {
  ...
  public fetchDailyMarketPrices(numberOfDays: number, date: Date, baseSymbol = 'USD'): Promise<MarketDataSample[]> {
    ...
    return new Promise((resolve, reject) => {
      this.algoSelector(numberOfDays, TimeUnit.Days, date, baseSymbol).then(marketSample => {
  ...
  public fetchHourlyMarketPrices(numberOfHours: number, date: Date, baseSymbol = 'USD'): Promise<MarketDataSample[]> {
    ...
    return new Promise((resolve, reject) => {
      this.algoSelector(numberOfHours, TimeUnit.Hours, date, baseSymbol).then(marketSample => {
  ...
  public fetchMinutesMarketPrices(numberOfMinutes: number, date: Date, baseSymbol = 'USD'): Promise<MarketDataSample[]> {
    ...
    return new Promise((resolve, reject) => {
      this.algoSelector(numberOfMinutes, TimeUnit.Minutes, date, baseSymbol).then(marketSample => {
  ...
  private algoSelector(numberOfMinutes: number, timeUnit: TimeUnit, date: Date, baseSymbol = 'USD'): Promise<MarketDataSample[]> {
    return new Promise((resolve, reject) => {
      ...
      if (timeUnit === 'days') {
        promise = cryptocompare.histoDay(this.coinProtocol.symbol.toUpperCase(), baseSymbol, {
          limit: numberOfMinutes - 1,
          timestamp: date
        })
      } else if (timeUnit === 'hours') {
        promise = cryptocompare.histoHour(this.coinProtocol.symbol.toUpperCase(), baseSymbol, {
          limit: numberOfMinutes - 1,
          timestamp: date
        })
      } else if (timeUnit === 'minutes') {
        promise = cryptocompare.histoMinute(this.coinProtocol.symbol.toUpperCase(), baseSymbol, {
          limit: numberOfMinutes - 1,
          timestamp: date
        })
      } else {
