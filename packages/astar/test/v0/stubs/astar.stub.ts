...
import { AstarProtocol } from '../../../src/v0'
import { ProtocolHTTPStub, TestProtocolSpec } from '../implementations'

export class AstarProtocolStub implements ProtocolHTTPStub {
  public async registerStub(testProtocolSpec: TestProtocolSpec, protocol: AstarProtocol): Promise<void> {
  ...
  public async noBalanceStub(testProtocolSpec: TestProtocolSpec, protocol: AstarProtocol): Promise<void> {
    const protocolOptions = await protocol.getOptions()
  ...
  private async registerDefaultStub(testProtocolSpec: TestProtocolSpec, protocol: AstarProtocol): Promise<void> {
    const protocolOptions = await protocol.getOptions()
