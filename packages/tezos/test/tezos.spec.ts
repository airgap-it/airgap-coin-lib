...
import { RunOperationMetadata, TezosProtocol } from '../src/protocol/TezosProtocol'
import { TezosOriginationOperation } from '../src/protocol/types/operations/Origination'
import { TezosRevealOperation } from '../src/protocol/types/operations/Reveal'
import { TezosOperation } from '../src/protocol/types/operations/TezosOperation'
import { TezosTransactionOperation } from '../src/protocol/types/operations/Transaction'
import { TezosOperationType } from '../src/protocol/types/TezosOperationType'
import { TezosWrappedOperation } from '../src/protocol/types/TezosWrappedOperation'
...
describe(`ICoinProtocol Tezos - Custom Tests`, () => {
  ...
  describe('TX Lists', () => {
    ...
    beforeEach(async () => {
      const res = await new TezosProtocolStub().registerStub(new TezosTestProtocolSpec(), tezosLib)
      getStub = res.getStub
      ...
      expect(transactions.map((transaction) => ({ ...transaction, network: undefined }))).to.deep.eq([
        {
          ...
          protocolIdentifier: await tezosLib.identifier,
  ...
  describe('KT1 Prepare/Sign', () => {
    ...
    it('will properly sign a TX to a KT1 address', async () => {
      const signedTezosTx = await tezosLib.signWithPrivateKey(tezosProtocolSpec.wallet.privateKey, {
        binaryTransaction:
  ...
  describe('Address Init', () => {
    ...
    describe('Spend', () => {
      ...
      it('will correctly calculate the max transaction value for both revealed and unrevealed accounts', async () => {
        ...
        for (let config of configs) {
          ...
          const maxFee = new BigNumber(estimatedFeeDefaults.medium).shiftedBy(await tezosLib.decimals).toNumber()
    ...
    it('will prepare an FA 1.2 transaction', async () => {
      ...
      postStub
        .withArgs(
          `${protocol.jsonRPCAPI}/chains/main/blocks/head/context/contracts/${await protocol.getContractAddress()}/script/normalized`
        )
        .returns(
          Promise.resolve({
            data: {
      ...
      getStub
        .withArgs(`${protocol.jsonRPCAPI}/chains/main/blocks/head/context/contracts/${await protocol.getContractAddress()}/entrypoints`)
        .returns(
          Promise.resolve({
            data: {
              entrypoints: {
    ...
    it('will prepare an FA2 transaction from public key', async () => {
            ...
            contents: [
              {
                ...
                fee: '35308',
                storage_limit: '60000',
                destination: await protocol.getContractAddress(),
                metadata
              }
            ]
          }
      ...
      expect(airGapTx.transactionDetails.storage_limit).to.equal('0')
      expect(airGapTx.transactionDetails.source).to.equal('tz1YvE7Sfo92ueEPEdZceNWd5MWNeMNSt16L')
      expect(airGapTx.transactionDetails.destination).to.equal(await protocol.getContractAddress())
      expect(airGapTx.transactionDetails.parameters).to.not.be.undefined
      expect(airGapTx.transactionDetails.parameters.entrypoint).to.equal('transfer')
