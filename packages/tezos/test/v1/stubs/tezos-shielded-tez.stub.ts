...
import { newAmount, SignedTransaction, UnsignedTransaction } from '@airgap/module-kit'
import * as sinon from 'sinon'

import { TezosShieldedTezProtocol } from '../../../src/v1'
import { TezosContractCode } from '../../../src/v1/types/contract/TezosContractCode'
import { ProtocolHTTPStub, TestProtocolSpec } from '../implementations'
import { TezosShieldedTezTestProtocolSpec } from '../specs/tezos-shielded-tez'

export class TezosShieldedTezProtocolStub implements ProtocolHTTPStub<TezosShieldedTezProtocol> {
  public async registerStub(testProtocolSpec: TezosShieldedTezTestProtocolSpec) {
    ...
    getStub
      .withArgs(`${protocolNetwork.rpcUrl}/chains/main/blocks/head/context/contracts/${testProtocolSpec.tezos.wallet.addresses[0]}/counter`)
    ...
    getStub
      .withArgs(`${protocolNetwork.rpcUrl}/chains/main/blocks/head/context/contracts/${testProtocolSpec.tezos.wallet.addresses[0]}/balance`)
      .returns(Promise.resolve({ data: 100000000 }))
    getStub
      .withArgs(
        `${protocolNetwork.rpcUrl}/chains/main/blocks/head/context/contracts/${testProtocolSpec.tezos.wallet.addresses[0]}/manager_key`
      )
      .returns(Promise.resolve({ data: { key: 'test-key' } }))

    postStub
      .withArgs(`${protocolNetwork.rpcUrl}/chains/main/blocks/head/context/contracts/${protocolNetwork.contractAddress}/script/normalized`)
      .returns(
    ...
    getStub
      .withArgs(`${protocolNetwork.rpcUrl}/chains/main/blocks/head/context/contracts/${protocolNetwork.contractAddress}/entrypoints`)
      .returns(
    ...
    getStub
      .withArgs(
        `${protocolNetwork.rpcUrl}/chains/main/blocks/head/context/contracts/${protocolNetwork.contractAddress}/single_sapling_get_diff`
      )
      .returns(
        Promise.resolve({
          data: {
  ...
  public async noBalanceStub(testProtocolSpec: TezosShieldedTezTestProtocolSpec) {
    const protocolNetwork = await testProtocolSpec.lib.getNetwork()

    sinon
      .stub(testProtocolSpec.lib, 'getBalanceOfPublicKey')
      .withArgs(sinon.match.any)
      .returns(Promise.resolve({ total: newAmount(0, 'blockchain') }))
  ...
  public async transactionListStub(
    testProtocolSpec: TestProtocolSpec<TezosShieldedTezProtocol, string, UnsignedTransaction, SignedTransaction>,
    address: string
  ): Promise<any> {}
}
