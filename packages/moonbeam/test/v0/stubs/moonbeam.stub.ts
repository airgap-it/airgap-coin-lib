...
import { MoonbeamProtocol } from '../../../src/v0'
import { ProtocolHTTPStub, TestProtocolSpec } from '../implementations'

export class MoonbeamProtocolStub implements ProtocolHTTPStub {
  public async registerStub(testProtocolSpec: TestProtocolSpec, protocol: MoonbeamProtocol): Promise<void> {
  ...
  public async noBalanceStub(testProtocolSpec: TestProtocolSpec, protocol: MoonbeamProtocol): Promise<void> {
    const protocolOptions = await protocol.getOptions()
  ...
  private async registerDefaultStub(testProtocolSpec: TestProtocolSpec, protocol: MoonbeamProtocol): Promise<void> {
    const protocolOptions = await protocol.getOptions()
