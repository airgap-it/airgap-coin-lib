...
export class MetadataV13Module extends SCALEClass {
  public static decode<C extends SubstrateProtocolConfiguration>(
    ...
    const calls = decoder.decodeNextOptional((configuration, runtimeVersion, hex) =>
      SCALEArray.decode(configuration, runtimeVersion, hex, MetadataV11Call.decode)
    )
    const events = decoder.decodeNextOptional((configuration, runtimeVersion, hex) =>
      SCALEArray.decode(configuration, runtimeVersion, hex, MetadataV11Event.decode)
    )
