...
import { SubstrateSignedTransaction } from '@airgap/substrate/v1'
...
import { AcurastTestProtocolSpec } from './specs/acurast'

// use chai-as-promised plugin
chai.use(chaiAsPromised)
const expect = chai.expect

const protocols = [new AcurastTestProtocolSpec()]
    ...
    describe(`Protocol ${protocol.name}`, () => {
      ...
      describe(`Sign Transaction`, () => {
        ...
        it('signTransactionWithSecretKey - Is able to sign a transaction using a SecretKey', async () => {
          ...
          const txs: SubstrateSignedTransaction[] = []
      ...
      describe(`Extract TX`, () => {
        it('getDetailsFromTransaction - Is able to extract all necessary properties from an unsigned TX', async () => {
          for (const tx of protocol.txs) {
            ...
            expect(airgapTx.arbitraryData, 'arbitraryDetails should not exist').to.be.undefined
          }
        })
