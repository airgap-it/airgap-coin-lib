...
import { BaseNodeClient } from './BaseNodeClient'

class BaseRPCDataGetL1Fee extends EthereumRPCData {
  public static methodName: string = 'getL1Fee'
  private readonly bytes: Buffer

  constructor(tx: EthereumUnsignedTransaction) {
    super(`${BaseRPCDataGetL1Fee.methodName}(bytes)`)
    this.bytes =
      tx.ethereumType === 'raw'
...
export class HttpBaseNodeClient extends HttpEthereumNodeClient implements BaseNodeClient {
  constructor(baseURL: string, headers?: any) {
    super(baseURL, headers)
  }

  public async getL1Fee(contractAddress: string, tx: EthereumUnsignedTransaction): Promise<BigNumber> {
    const data = new BaseRPCDataGetL1Fee(tx)
