...
import { concat, fromHex, toHex } from './buffer'
import { Principal } from './principal'
import * as cbor from './cbor'
import { blsVerify as BLSVer } from './bls'
...
export class Certificate {
  ...
  private async _checkDelegationAndGetKey(d?: Delegation): Promise<ArrayBuffer> {
    ...
    const ranges: Array<[Principal, Principal]> = ranges_arr.map((v) => [Principal.fromUint8Array(v[0]), Principal.fromUint8Array(v[1])])
...
export async function reconstruct(t: HashTree): Promise<ArrayBuffer> {
  switch (t[0]) {
    ...
    case NodeId.Labeled:
      return hash(concat(domain_sep('ic-hashtree-labeled'), t[1] as ArrayBuffer, await reconstruct(t[2] as HashTree)))
    case NodeId.Fork:
      return hash(concat(domain_sep('ic-hashtree-fork'), await reconstruct(t[1] as HashTree), await reconstruct(t[2] as HashTree)))
    default:
 ...
 * @param tree
 */
export function lookup_path(path: Array<ArrayBuffer | string>, tree: HashTree): ArrayBuffer | undefined {
  if (path.length === 0) {
    switch (tree[0]) {
      case NodeId.Leaf: {
