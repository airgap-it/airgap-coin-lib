...
export class CosmosTransaction implements JSONConvertible, RPCConvertible, Encodable {
  ...
  public toAirGapTransactions(identifier: ProtocolSymbols, network: CosmosProtocolNetwork): IAirGapTransaction[] {
    const fee = this.fee.amount.map((value) => new BigNumber(value.amount)).reduce((prev, next) => prev.plus(next))

    return this.messages
      .map((message: CosmosMessage) => message.toAirGapTransaction(identifier, network, fee.toString(10)))
