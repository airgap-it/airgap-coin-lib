...
export class KusamaProtocolStub implements ProtocolHTTPStub {
  public async registerStub(testProtocolSpec: TestProtocolSpec, protocol: KusamaProtocol): Promise<void> {
  ...
  public async noBalanceStub(testProtocolSpec: TestProtocolSpec, protocol: KusamaProtocol): Promise<void> {
    const protocolOptions = await protocol.getOptions()
  ...
  private async registerDefaultStub(testProtocolSpec: TestProtocolSpec, protocol: KusamaProtocol): Promise<void> {
    const protocolOptions = await protocol.getOptions()
    ...
    sinon.stub(protocolOptions.nodeClient, 'getRuntimeVersion').returns(Promise.resolve({ specVersion: 2030, transactionVersion: 1 }))
  }
}
