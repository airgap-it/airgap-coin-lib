...
describe('Validators', () => {
  describe('custom validators', () => {
    it('should validate BigNumber', async () => {
      ...
      expect(validationErrors).to.deep.equal({ test: ['Test is not of type "BigNumber"'] })
      validationErrors = await validate({ test: { isBigNumber: function() {} } }, constraints)
      expect(validationErrors).to.deep.equal({ test: ['Test is not of type "BigNumber"'] })
  ...
  describe('Sync Scheme', () => {
    ...
    it('should accept validate sync scheme', async () => {
      let validationErrors = await validateSyncScheme(validScheme)
      expect(validationErrors).to.be.undefined
    })

    it('should reject scheme if version is invalid', async () => {
      let myScheme = { ...validScheme }
      let invalidValues: { value: any; expected: any }[] = [
      ...
      for (let invalidValue of invalidValues) {
        ...
        let validationErrors = await validateSyncScheme(myScheme)
